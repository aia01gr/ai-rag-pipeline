...
```
PDFs → chunks_with_sentencesplitter.py → text chunks (output/)
                              ↓
                    embeddings_with_voyage.py → EmbeddedChunk objects (output/)
                              ↓
                      vector_database.py → ChromaDB (chroma_db/)
                              ↓
                    mcp_server.py / rag_pipeline_local_with_ollama.py → Claude Desktop / Ollama
```
...
| File | Class/Purpose |
|------|---------------|
| `chunks_with_sentencesplitter.py` | PDF text extraction with OCR fallback (pdfplumber + pytesseract) |
| `embeddings_with_voyage.py` | `EmbeddingGenerator` — pluggable providers: `voyage`, `openai`, `sentence-transformers` |
| `vector_database.py` | `VectorDatabase` — ChromaDB wrapper; cosine similarity search |
| `rag_pipeline_local_with_ollama.py` | `RAGPipeline` — combines vector search + Ollama (`qwen2.5:32b` default) for full Q&A |
...
- **Batch processing**: `embeddings_with_voyage.py` and `chunks_with_sentencesplitter.py` support checkpoint/resume for large PDF sets
