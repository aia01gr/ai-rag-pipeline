...
7. [Διαχείριση PDFs](#διαχείριση-pdfs)
8. [Χρήσιμα Commands](#χρήσιμα-commands)
...
6. **Scripts διαχείρισης PDFs**: Αφαίρεση PDF από ChromaDB και embedded_chunks.json.
...
### Αφαίρεση PDF (και από τα δύο — συνιστάται)
...
Αφαιρεί chunks από ChromaDB **και** `embedded_chunks.json` με μία επιβεβαίωση.

```
Αναζήτηση 'αρχείο.pdf' ...
  ChromaDB:          58061 chunks συνολικά
  → βρέθηκαν:        77 chunks
  embedded_chunks:   58852 chunks συνολικά
  → βρέθηκαν:        77 chunks

Αφαίρεση 77 chunks από ChromaDB και 77 από embedded_chunks.json; [y/N] y
  ChromaDB:        αφαιρέθηκαν 77 chunks → έμειναν 57984
  embedded_chunks: αφαιρέθηκαν 77 chunks → έμειναν 58775
...
- Τα scripts δέχονται μόνο το όνομα αρχείου, όχι το πλήρες path.
- Αν το αρχείο δεν βρεθεί, εμφανίζουν λίστα με τα διαθέσιμα PDFs.
...
| Script | Περιγραφή |
|---|---|
| `remove_pdf.py` | Αφαίρεση PDF από ChromaDB **και** embedded_chunks.json |
| `remove_pdf_chroma.py` | Αφαίρεση PDF μόνο από ChromaDB |
| `remove_pdf_chunks.py` | Αφαίρεση PDF μόνο από embedded_chunks.json |
